...
Add these to your `.env.local` (NEVER commit real values — `.env*` is gitignored):

```
RAZORPAY_KEY_ID=<your_rzp_key_id>
RAZORPAY_KEY_SECRET=<your_rzp_key_secret>
RAZORPAY_PAYOUT_ACCOUNT_NUMBER=<your_razorpayx_account_number>
```

Get your Account ID from: https://dashboard.razorpay.com/app/settings/account-settings

> ⚠️ **Security:** A live Razorpay key was previously committed in this file. Treat
> it as compromised and **rotate it immediately** in the Razorpay dashboard
> (Settings → API Keys → Regenerate). Secrets must only live in environment
> variables, never in source or docs.
