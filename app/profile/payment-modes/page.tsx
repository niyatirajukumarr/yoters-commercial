  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f0eb', paddingBottom: 40 }}>
      ...
      <div style={{ padding: '20px 16px' }}>
        ...
        <motion.div initial="hidden" animate="visible" variants={stagger} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {paymentMethods.map(method => (
            <motion.div key={method.id} variants={staggerItem} {...hoverLift} style={{ background: 'white', borderRadius: 14, padding: '16px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <div>
              ...
              <motion.button {...hoverScale} style={{ background: '#E8334A', color: 'white', border: 'none', borderRadius: 8, padding: '8px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                Manage
              </motion.button>
            </motion.div>
          ))}
        ...
        {/* Add New */}
        <motion.button {...hoverScale} style={{ marginTop: 20, width: '100%', padding: '14px', background: 'white', border: '2px dashed #ddd', borderRadius: 14, color: '#E8334A', fontWeight: 600, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
