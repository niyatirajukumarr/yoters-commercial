...
// Rotatable WebGL sphere of image discs — items sit on the vertices of a
// subdivided icosahedron; dragging spins the sphere and it snaps the nearest
// disc to face the camera. Adapted from the "Infinite Menu" community
// component (21st.dev/@deleted_reactbits), with the demo's external-link
// button replaced by an onItemClick callback, and an onUnsupported escape
// hatch so callers can fall back to a plain list where WebGL2 is unavailable.
...
function createAndSetupTexture(
  gl: WebGL2RenderingContext, minFilter: number, magFilter: number, wrapS: number, wrapT: number
): WebGLTexture {
...
class ArcballControl {
  ...
  // Kept so the caller can detach them on unmount — without this the
  // listeners (and the canvas they close over) leak on every remount.
  ...
  constructor(private canvas: HTMLCanvasElement, private updateCallback: UpdateCallback = () => undefined) {
    ...
    this.listeners.forEach(([type, fn]) => canvas.addEventListener(type, fn))
    canvas.style.touchAction = 'none'
  }

  public dispose(): void {
    this.listeners.forEach(([type, fn]) => this.canvas.removeEventListener(type, fn))
    this.listeners = []
  }
...
class InfiniteGridMenu {
  ...
  // Stops the rAF loop and detaches pointer listeners. Without this the loop
  // keeps running (and re-scheduling) after the component unmounts.
  public dispose(): void {
    this.disposed = true
  ...
  private initTexture(): void {
    ...
    Promise.all(
      this.items.map(item => new Promise<HTMLImageElement | null>(resolve => {
        const img = new Image()
        img.crossOrigin = 'anonymous'
        img.onload = () => resolve(img)
        // A single 404 / CORS failure shouldn't reject the whole atlas and
        // leave every disc untextured — skip just that cell instead.
      ...
      try {
        ...
        // Tainted canvas (an image served without CORS headers) — the discs
        // stay blank rather than the page crashing.
        console.error('InfiniteMenu: texture upload failed', e)
      }
    })
  }
...
interface InfiniteMenuProps {
  ...
  /** Called when the focused disc's action button is tapped. */
  onItemClick?: (item: InfiniteMenuItem) => void
  /** Rendered instead of the canvas when WebGL2 isn't available. */
  onUnsupported?: () => void
}

const InfiniteMenuComponent: FC<InfiniteMenuProps> = ({ items = [], onItemClick, onUnsupported }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('InfiniteMenu: WebGL2 unavailable', e)
      onUnsupported?.()
      return
    }
      ...
      {activeItem && (
        <div className="shrink-0 flex flex-col items-center gap-1 pt-2">
          ...
          {activeItem.description && (
            <span className="select-none text-center text-[11px] text-[color:var(--muted)]">
              {activeItem.description}
            </span>
          )}
          <button
...
export const InfiniteMenu = InfiniteMenuComponent
